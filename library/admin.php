...
$sweetcaptcha_options = array(
	...
	'sweetcaptcha_additional_settings_top' => array('title' => 'Additional Settings'),
	...
	'sweetcaptcha_form_gf' => array('title' => __('sweetCaptcha for Gravity Forms', 'sweetcaptcha'), 'description' => __('Enable sweetCaptcha for Gravity Forms.', 'sweetcaptcha')),
	...
	'sweetcaptcha_additional_settings_bottom' => array('title' => __('Additional Settings', 'sweetcaptcha')),
);
...
function sweetcaptcha_admin_notices() {
	...
	if (sweetcaptcha_is_registered()) {
		...
		$sweetcaptcha_instance->check_access();
	} else {
		echo '<div class="error sweetcaptcha" style="text-align: center; ">
      <p style="color: red; font-size: 14px; font-weight: bold;">' . __('Your sweetCaptcha plugin is not setup yet', 'sweetcaptcha')
		. '</p><p>' . __('Click <a href="options-general.php?page=sweetcaptcha">here</a> to finish setup.', 'sweetcaptcha') . '</p></div>'
		;
		sweetcaptcha_popup_setup();
	}
	echo '<style type="text/css">body.wp-admin #wpfooter {display: none}</style>';
}

function sweetcaptcha_popup_setup() { 
...
?>
	<style>
		#sweetcaptcha-popup-header {background-color: #D81378; border-color: #D81378;}
		#sweetcaptcha-popup-header:before {color:#D81378;}
	</style>
	<script type="text/javascript">
		//<![CDATA[
		(function($) {
			var setup = function() {
				$('#toplevel_page_sweetcaptcha').pointer({
						content: '<h3 id="sweetcaptcha-popup-header">sweetCaptcha needs to be configured</h3><p>Configure sweetCaptcha <a href="options-general.php?page=sweetcaptcha">here</a>.</p>',
						position: {
...
function sweetcaptcha_admin_menu() {
	...
	add_menu_page(__('sweetCaptcha', 'sweetcaptcha'), __('sweetCaptcha', 'sweetcaptcha'), 'manage_options', 'sweetcaptcha', 'sweetcaptcha_options_page', SWEETCAPTCHA_URL . '/images/menu-icon.png');
}
...
function sweetcaptcha_options_page() {
	...
	if (!current_user_can('manage_options')) {
		wp_die(__('You do not have sufficient permissions to access this page.', 'sweetcaptcha'));
	}
...
function sweetcaptcha_register_form() {
	...
	if ((isset($_POST[$hidden_field_name])) && ($_POST[$hidden_field_name] == 'Y')) {
		...
		if ($result['error']) {
			...
			echo "<script type=\"text/javascript\" language=\"javascript\">jQuery('div.error.sweetcaptcha').hide();</script>";
			return;
		}
	} else {
	...
	$email = isset($_POST['email']) ? json_encode($_POST['email']) : '';

	//jQuery('<div><input type="text" class="field" name="dynamic[]" value="' + i + '" /></div>').fadeIn('slow').appendTo('.inputs');
	$form_html .= '<style type="text/css">span.error{color:red;}</style>';
...
function sweetcatpcha_main_settings($ignore_post = false) {
	...
	if ((!$ignore_post) && (isset($_POST[$hidden_field_name])) && ($_POST[$hidden_field_name] == 'Y')) {
		...
		if (swtcptcf_settings_save()) {
			...
			if (sweetcaptcha_is_registered()) {
				$saved_html .= "<script type=\"text/javascript\" language=\"javascript\">jQuery( 'div.error.sweetcaptcha' ).hide();</script>";
			}
		}
...
function swtcptcf_settings_save() {
	...
	if ($swtcptcf_options_submit['swtcptcf_action_after_send'] == 0 && ( trim($swtcptcf_options_submit['swtcptcf_redirect_url']) == "" || !preg_match('@^(?:http://)?([^/]+)@i', trim($swtcptcf_options_submit['swtcptcf_redirect_url'])) )) {
	...
	if ('user' == $swtcptcf_options_submit['swtcptcf_select_email']) {
		if (get_user_by('login', $swtcptcf_options_submit['swtcptcf_user_email'])) {
